...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['proposal_id'], $_POST['action'])) {
    ...
    // OSAS approval logic
    if ($action === 'approve') {
        // Kung OSAS na ang role, set to Approved na
        $status = 'Approved';
        $new_level = 'Completed'; // Optional: para malaman na tapos na ang approval process
    } elseif ($action === 'disapprove') {
        $status = 'Disapproved by OSAS';
        $new_level = 'OSAS';
    } else {
        die("Invalid action");
    }
    ...
    if ($stmt->execute()) {
        header("Location: osas.php"); // ← palitan ito sa tamang file
        exit;
    } else {
...
<header class="topbar">
    ...
    <nav>
        ...
        <div class="admin-info">
            <span><?php echo htmlspecialchars($_SESSION['role'] ?? 'OSAS'); ?></span>
            ...
            <tr>
                ...
                <td>
                    <form method="post" action="" style="margin:0;">
                        ...
                        <button type="submit" name="action" value="approve" class="btn btn-success" onclick="showSuccessModal()">Approve</button>
  ...
  <div class="modal-dialog modal-dialog-centered">
    <div class="modal-content text-center">
      <div class="modal-header border-0">
        <h5 class="modal-title w-100" id="successModalLabel">Approval Successful</h5>
      </div>
